      ...
      <div className="lg:grid grid-cols-3 gap-6 mt-10">
        {paginatedBlogs.map((blog) => (
          <motion.div
            ...
            className="dark:text-stone-100 text-stone-900 border-[1px] md:mt-8 mt-6 border-neutral-400 dark:border-stone-700 lightBackImage backImage dark:backdrop-blur-md rounded-xl p-6"
          >
            ...
            <div>
              <span className="flex text-xs mt-2 dark:text-stone-500 text-stone-600 items-center justify-start gap-3">
                <span className="dark:text-[#c8f31d] text-green-700">
                  <FaCalendar />
                </span>
                <span>
            ...
            <div className="grid grid-cols-2 sm:flex text-xs mt-3 items-center justify-start gap-2">
              {(blog.techs ?? []).map((tech, index) => (
                <span
                  key={index}
                  className="px-2 flex items-center gap-1 py-1 border-[1px] border-neutral-400 dark:border-stone-700 rounded-xl"
                >
                  <CiBellOn /> {tech}
                </span>
              ))}
            ...
            <Link href={`/blog/${blog.slug.current}`}>
              <button className="flex font-semibold items-center justify-center gap-3 py-2 w-full group relative overflow-hidden rounded-xl border-[1px] border-neutral-300 dark:border-stone-600 mt-4 dark:hover:bg-stone-800 transition-all duration-700">
                <span className="relative z-10 flex items-center gap-3 justify-center">
                  ...
                  <span className="dark:text-[#c8f31d] text-green-800">
                    <RxExternalLink />
                  </span>
                </span>
                <span
                  className="absolute inset-0 bg-neutral-200 dark:bg-stone-700 duration-700 transition-transform 
                               ease-out group-hover:translate-x-0 -translate-x-full"
                ></span>
              </button>
            </Link>
          </motion.div>
