...
const Navbar = memo(function Navbar() {
  ...
  return (
    <>
      <nav
        className={`hidden sm:flex fixed top-6 left-1/2 -translate-x-1/2 z-40 2xl:px-24 transition-all duration-[1s] rounded-full ${
          navScrolled
            ? "dark:bg-stone-900/10 bg-stone-200 xl:w-[50%] md:w-[70%]flex items-center justify-between fixed backdrop-blur-md"
            : "bg-transparent w-[90%]"
        }`}
      >
        <div className="container px-12 xl:px-0 gap-6 h-16 flex items-center justify-between transition-all duration-[3s]">
          <div
            className={`hidden sm:flex tracking-widest text-2xl dark:text-stone-100 text-stone-900 font-semibold 
                          cursor-pointer transition-all duration-1000 font-sans`}
          >
